...
// removed key bindings; keep only button mnemonics

// Based on
// https://stackoverflow.com/questions/24034747/how-to-implement-mouselistener-on-a-particular-shape

class HexGUI extends JPanel implements MouseListener {
  ...
  // Random sequence now handled inside Hex
  ...
  final int n;
  enum Mode { HUMAN, RANDOM, HEURISTIC }
  private JComboBox<Mode> redSelect;
  private JComboBox<Mode> blueSelect;
  ...
  public void mouseClicked(MouseEvent e) {
    if (modeFor(hex.currentPlayer()) != Mode.HUMAN)
      return;
  ...
  static void createAndShowGUI() {
    ...
    HexGUI board = new HexGUI(size);
    JPanel root = new JPanel(new BorderLayout());
    root.add(board, BorderLayout.CENTER);
    root.add(board.makeControls(), BorderLayout.SOUTH);
    f.setContentPane(root);
    // Allocate space for the board plus a small control strip
    f.setSize((int) HexGUI.centerX(size + 1, size + 2),
        (int) HexGUI.centerY(size + 1, size + 2) + 80);
  ...
  JPanel makeControls() {
    ...
    redSelect = new JComboBox<>(Mode.values());
    redSelect.setSelectedItem(Mode.HUMAN);
    p.add(redSelect);

    p.add(new JLabel("Blue:"));
    blueSelect = new JComboBox<>(Mode.values());
    blueSelect.setSelectedItem(Mode.HUMAN);
    p.add(blueSelect);
    ...
    btnPlay.setMnemonic('Y');
    btnPlay.addActionListener(ev -> {
      play();
    });
    p.add(btnPlay);

    // Spacer between game controls and debugging toggles
    p.add(Box.createHorizontalStrut(36));

    JCheckBox cbLabels = new JCheckBox("Labels", printLabels);
    cbLabels.addActionListener(ev -> {
      printLabels = cbLabels.isSelected();
      repaint();
    });
    p.add(cbLabels);

    JCheckBox cbColors = new JCheckBox("Colors", printColors);
    cbColors.addActionListener(ev -> {
      printColors = cbColors.isSelected();
      repaint();
    });
    p.add(cbColors);

    JCheckBox cbCoords = new JCheckBox("Coords", printCoords);
    cbCoords.addActionListener(ev -> {
      printCoords = cbCoords.isSelected();
      repaint();
    });
    p.add(cbCoords);
  ...
  // Plays AI moves while it's not a human's turn and no winner yet
  void play() {
    while (hex.winner() == Hex.Player.NOONE && modeFor(hex.currentPlayer()) != Mode.HUMAN) {
      Mode mode = modeFor(hex.currentPlayer());
      boolean ok = (mode == Mode.RANDOM) ? hex.randomMove() : hex.heuristicMove();
      if (!ok)
        break;
    }
    repaint();
  ...
  private Mode modeFor(Hex.Player p) {
    if (p == Hex.Player.RED)
      return (Mode) redSelect.getSelectedItem();
    else if (p == Hex.Player.BLUE)
      return (Mode) blueSelect.getSelectedItem();
    return Mode.HUMAN;
  }
